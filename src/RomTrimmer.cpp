...
void RomTrimmer::defineCommandLineOptions(cxxopts::Options& opts)
{
    ...
    // Nova opção para extensões personalizadas
    ("e,extensions", "Extensões personalizadas (ex: nds,gba,nes,smc)",
     cxxopts::value<std::string>())

    // Nova opção para arquivos compactados
    ("compressed", "Processar arquivos compactados (ZIP, 7Z, RAR)")
    ("extract-only", "Apenas extrair arquivos compactados sem processar")

    // Opção para diretório de extração temporária
    ("extract-dir", "Diretório para extração temporária",
    ...
    // Processar extensões personalizadas
    ...
    // ==================== OPCIONAL: REZIP ====================
    ("rezip", "Recomprimir ROMs após trimming (ZIP)")
    ("rezip-format", "Formato de recompactação (zip, 7z, tar.gz)",
     cxxopts::value<std::string>()->default_value("zip"))
    ("rezip-level", "Nível de compressão (0-9)",
     cxxopts::value<int>()->default_value("5"))
    ("keep-original", "Manter arquivo original após rezip");
}
...
void RomTrimmer::processCommandLineOptions(const cxxopts::ParseResult& result)
{
    ...
    {
        std::string extStr = result["extensions"].as<std::string>();
        processCustomExtensions(extStr);
    }

    // Processar opções de compactação
    if (result.count("compressed"))
    {
        processCompressed = true;
    }

    if (result.count("extract-only"))
    {
        extractCompressed = true;
        processCompressed = true;
    }

    if (result.count("extract-dir"))
    {
        std::string dir = result["extract-dir"].as<std::string>();
        if (!dir.empty())
        {
            tempExtractDir = dir;
    ...
    // ==================== CONFIGURAÇÃO DE REZIP ====================
    if (result.count("rezip"))
    {
        rezipAfterTrim = true;
        logger->log("Rezip habilitado", LogLevel::INFO);
    }

    if (result.count("rezip-format"))
    {
        rezipFormat = result["rezip-format"].as<std::string>();
        if (rezipFormat != "zip" && rezipFormat != "7z" && rezipFormat != "tar.gz")
        {
            logger->log("Formato de recompactação inválido, usando ZIP", LogLevel::WARNING);
            rezipFormat = "zip";
        }
    }

    if (result.count("rezip-level"))
    {
        rezipLevel = result["rezip-level"].as<int>();
        rezipLevel = std::clamp(result["rezip-level"].as<int>(), 0, 9);
    }

    keepOriginalAfterRezip = result.count("keep-original") > 0;
}
    ...
    for (const auto& inputPath : options.inputPaths)
    {
        try
        {
            ...
            if (fs::is_regular_file(inputPath))
            {
                ...
                if (processCompressed && isCompressedFile(inputPath))
                {
                    if (!processCompressedArchive(inputPath, allFiles))
                    {
                        logger->log("Falha ao processar arquivo compactado: " +
                                    inputPath.string(), LogLevel::ERROR);
                    }
                    continue;
                }

                // Arquivo único - verificar extensão
                if (isSupportedFileExtension(inputPath, customExtensions))
                {
    ...
    try
    {
        if (options.recursive)
        {
            ...
            for (const auto& entry : fs::recursive_directory_iterator(dir))
            {
                ...
                {
                    // Verificar se é arquivo compactado
                    if (processCompressed && isCompressedFile(entry.path()))
                    {
                        if (!processCompressedArchive(entry.path(), allFiles))
                        {
                            logger->log("Falha ao processar arquivo compactado: " +
                                        entry.path().string(), LogLevel::ERROR);
                        }
                        continue;
                    }

                    // Verificar extensão
                    if (isSupportedFileExtension(entry.path(), customExtensions))
                    {
...
bool RomTrimmer::isSupportedFileExtension(const fs::path& filePath)
{
    ...
    return isSupportedFileExtension(filePath, customExtensions);
}

bool RomTrimmer::isSupportedFileExtension(const fs::path& filePath,
        const std::unordered_set<std::string>& extensions)
{
    // Se não há extensões personalizadas definidas, usar lista padrão
    if (extensions.empty())
    {
        static const std::unordered_set<std::string> defaultExtensions =
        {
        ...
        return defaultExtensions.find(ext) != defaultExtensions.end();
    }
    else
    {
...
bool RomTrimmer::handleActualTrim(const fs::path& filePath,
    ...
    fs::path trimmedPath = determineOutputPath(filePath);

    if (!writeTrimmedFile(trimmedPath, data, trimPoint)) {
        return false;
    }
    ...
    // ==================== REZIP ====================
    if (rezipAfterTrim && !extractCompressed) {
        logger->log("Iniciando recompactação...", LogLevel::INFO);

        if (rezipFile(trimmedPath)) {
            logger->log("Arquivo recomprimido com sucesso", LogLevel::INFO);
            stats.rezipped = true;

            // Remover arquivo original após rezip se configurado
            if (!keepOriginalAfterRezip) {
                try {
                    fs::remove(trimmedPath);
                    logger->log("Arquivo original removido após rezip", LogLevel::DEBUG);
                } catch (const std::exception& e) {
                    logger->log("Não foi possível remover arquivo original: " +
                               std::string(e.what()), LogLevel::WARNING);
                }
            }
        } else {
            logger->log("Falha na recompactação", LogLevel::ERROR);
        }
    }
...
void RomTrimmer::printSummary() const
{
    ...
    // Estatísticas de rezip
    if (rezipAfterTrim) {
        std::cout << "\n" << TR("REZIP_STATISTICS") << ":\n";
        std::cout << "  Formato: " << rezipFormat << "\n";
        std::cout << "  Nível de compressão: " << rezipLevel << "\n";

        int filesRezipped = 0;
        for (const auto& stats : fileStats) {
            if (stats.rezipped) filesRezipped++;
        }

        std::cout << "  Arquivos recomprimidos: " << filesRezipped << "\n";
    }
...
void RomTrimmer::cleanup()
{
    ...
    if (!tempExtractDir.empty() && fs::exists(tempExtractDir))
    {
        try
        {
            if (!extractCompressed)   // Só limpar se não for apenas extração
            {
                fs::remove_all(tempExtractDir);
                logger->log("Diretório temporário limpo: " + tempExtractDir.string(),
                            LogLevel::DEBUG);
            }
        }
        catch (const std::exception& e)
        {
            logger->log("Não foi possível limpar diretório temporário: " +
                        std::string(e.what()), LogLevel::WARNING);
        }
    }
...
void RomTrimmer::processCustomExtensions(const std::string& extensions)
{
    ...
    {
        // Adicionar ponto se não tiver
        if (ext.empty()) continue;
        if (ext[0] != '.') ext = "." + ext;

        // Converter para minúsculas
        std::transform(ext.begin(), ext.end(), ext.begin(), ::tolower);
        customExtensions.insert(ext);
    }

    if (options.verbose && !customExtensions.empty())
    {
        logger->log("Extensões personalizadas definidas:", LogLevel::DEBUG);
        for (const auto& ext : customExtensions)
        {
            logger->log("  - " + ext, LogLevel::DEBUG);
        }
    }
}
// ==================== FUNÇÕES PARA ARQUIVOS COMPACTADOS ====================

bool RomTrimmer::isCompressedFile(const fs::path& filePath) const
{
    std::string ext = filePath.extension().string();
    std::transform(ext.begin(), ext.end(), ext.begin(), ::tolower);
    ...
    return compressedExtensions.find(ext) != compressedExtensions.end();
}

bool RomTrimmer::processCompressedArchive(const fs::path& archivePath,
        std::vector<fs::path>& allFiles)
{
    try
    {
        ...
        {
            tempExtractDir = fs::temp_directory_path() / "romtrimmer_extract";
            fs::create_directories(tempExtractDir);
        }

        // Criar subdiretório específico para este arquivo
        fs::path extractPath = tempExtractDir / archivePath.stem();
        ...
        logger->log("Extraindo arquivo compactado: " + archivePath.string(),
                    LogLevel::INFO);
        ...
        if (extension == ".zip")
        {
            extractionSuccess = extractZipArchive(archivePath, extractPath);
        }
        else if (extension == ".7z")
        {
            extractionSuccess = extract7zArchive(archivePath, extractPath);
        }
        else if (extension == ".rar")
        {
            extractionSuccess = extractRarArchive(archivePath, extractPath);
        }
        else
        {
            logger->log("Formato de compactação não suportado: " + extension,
                        LogLevel::ERROR);
            return false;
        }
        ...
        {
            logger->log("Falha ao extrair arquivo: " + archivePath.string(),
                        LogLevel::ERROR);
            return false;
        }

        logger->log("Extração concluída: " + archivePath.string(), LogLevel::INFO);

        // Se for apenas extração, não processar os arquivos extraídos
        if (extractCompressed)
        {
            logger->log("Arquivos extraídos para: " + extractPath.string(),
                        LogLevel::INFO);
            return true;
        }

        // Coletar arquivos extraídos
        for (const auto& entry : fs::recursive_directory_iterator(extractPath))
        {
            if (fs::is_regular_file(entry.path()))
            {
                ...
                {
                    allFiles.push_back(entry.path());
                    logger->log("Adicionando arquivo extraído: " + entry.path().string(),
                                LogLevel::DEBUG);
                }
            }
        }
    ...
    {
        logger->log("Erro ao processar arquivo compactado " + archivePath.string() +
                    ": " + e.what(), LogLevel::ERROR);
        return false;
    }
}

// Funções de extração (implementação simplificada)
bool RomTrimmer::extractZipArchive(const fs::path& archivePath, const fs::path& extractPath)
{
#ifdef _WIN32
    // Usar API Windows para ZIP
    return extractWithWindowsAPI(archivePath, extractPath, "ZIP");
#else
    // Usar system() para comandos externos (simplificado)
    std::string command = "unzip -q \"" + archivePath.string() + "\" -d \"" +
                          extractPath.string() + "\"";
...
bool RomTrimmer::extract7zArchive(const fs::path& archivePath, const fs::path& extractPath)
{
#ifdef _WIN32
    // Usar API Windows ou 7z.exe
    std::string command = "7z x \"" + archivePath.string() + "\" -o\"" +
                          extractPath.string() + "\" -y";
#else
    // Usar 7z ou p7zip
    std::string command = "7z x \"" + archivePath.string() + "\" -o\"" +
                          extractPath.string() + "\" -y";
...
bool RomTrimmer::extractRarArchive(const fs::path& archivePath, const fs::path& extractPath)
{
    std::string command = "unrar x \"" + archivePath.string() + "\" \"" +
                          extractPath.string() + "\" -y";
    int result = system(command.c_str());
    return result == 0;
}
...
bool RomTrimmer::rezipFile(const fs::path& trimmedPath) {
    try {
        if (!fs::exists(trimmedPath) || !fs::is_regular_file(trimmedPath)) {
            logger->log("Arquivo trimado não encontrado: " + trimmedPath.string(),
                       LogLevel::ERROR);
            return false;
        }

        fs::path archivePath = trimmedPath;

if (rezipFormat == "tar.gz") {
    archivePath += ".tar.gz";
} else {
    archivePath.replace_extension("." + rezipFormat);
}


        logger->log("Criando arquivo compactado: " + archivePath.string(),
                   LogLevel::INFO);

        bool success = false;

        if (rezipFormat == "zip") {
            success = createZipArchive(trimmedPath, archivePath);
        } else if (rezipFormat == "7z") {
            success = create7zArchive(trimmedPath, archivePath);
        } else if (rezipFormat == "tar.gz") {
            success = createTarGzArchive(trimmedPath, archivePath);
        }

        if (success) {
            size_t originalSize = fs::file_size(trimmedPath);
            size_t archiveSize = fs::file_size(archivePath);
            double compressionRatio = 100.0 * (1.0 - (double)archiveSize / originalSize);

            logger->log("Compactação concluída: " + formatBytes(archiveSize) +
                       " (redução de " + std::to_string((int)compressionRatio) + "%)",
                       LogLevel::INFO);
        }
        ...
        logger->log("Erro ao recomprimir arquivo: " + std::string(e.what()),
                   LogLevel::ERROR);
        return false;
    }
}

bool RomTrimmer::createZipArchive(const fs::path& filePath, const fs::path& zipPath) {
#ifdef _WIN32
    // Usar Windows API para ZIP
    std::string command = "powershell -Command \"Compress-Archive -Path '" +
                         filePath.string() + "' -DestinationPath '" +
                         zipPath.string() + "' -CompressionLevel Optimal\"";
#else
    // Usar zip no Linux/macOS
    std::string command = "zip -q -" + std::to_string(rezipLevel) + " \"" +
                         zipPath.string() + "\" \"" + filePath.string() + "\"";
#endif

    logger->log("Executando: " + command, LogLevel::DEBUG);

    int result = system(command.c_str());
    return result == 0;
}

bool RomTrimmer::create7zArchive(const fs::path& filePath, const fs::path& archivePath) {
    std::string command = "7z a -t7z -mx=" + std::to_string(rezipLevel) +
                         " \"" + archivePath.string() + "\" \"" +
                         filePath.string() + "\"";
...
bool RomTrimmer::createTarGzArchive(const fs::path& filePath, const fs::path& archivePath) {
    std::string command = "tar -czf \"" + archivePath.string() + "\" \"" +
                         filePath.string() + "\"";
